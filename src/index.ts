...
class LocalYouTubeMCPServer {
    ...
    private async loadStoredTokens(): Promise<UserTokens | null> {
        try {
    ...
    private async saveTokens(tokens: UserTokens): Promise<void> {
        try {
    ...
    private async initiateUserAuth(): Promise<string> {
        const scopes = [
    ...
    private async completeAuth(code: string): Promise<boolean> {
        try {
...
Waiting for authorization... (${(callbackError as Error).message})`,
                        },
                    ],
                };
            }
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `❌ Error during authentication setup: ${(error as Error).message}`,
                    },
                ],
                isError: true,
            };
...
🔧 Authentication file: ${this.tokensPath}`,
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `⚠️ Connected but unable to fetch channel details: ${(error as Error).message}`,
                        },
                    ],
                };
            }
    ...
    private async handleYouTubeOperation(operation: string, args: any) {
        switch (operation) {
            case 'youtube_search_videos':
    ...
    private async searchVideos(args: any) {
        ...
        if (published_after) searchParams.publishedAfter = published_after;
        if (published_before) searchParams.publishedBefore = published_before;
        ...
        const detailsMap = new Map(detailsResponse.data.items.map((item: any) => [item.id, item]));

        const enrichedVideos = videos.map((video: any) => {
            const details: any = detailsMap.get(video.id.videoId);
            return {
                id: video.id.videoId,
    ...
    private async getChannel(args: any) {
    ...
    private async listChannelVideos(args: any) {
    ...
    private async getMyChannel(args: any) {
    ...
    private async listMyVideos(args: any) {
    ...
    private async getPlaylists(args: any) {
    ...
    private async getPlaylistVideos(args: any) {
    ...
    private async getTrending(args: any) {
